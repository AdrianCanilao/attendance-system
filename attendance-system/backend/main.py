...
async def verify_face(
    ...
    try:
        ...
        # =====================================================
        # 4. SEND SELECTED FRAMES TO INSIGHTFACE SERVER
        # PORT 8002
        #
        # Keep all 8 frames for MediaPipe blink/liveness above,
        # but use 4 evenly-spread frames for the expensive
        # InsightFace recognition step.
        # =====================================================

        recognition_results = []

        if len(frames) >= 8:
            recognition_indices = [0, 2, 5, 7]
        else:
            recognition_indices = list(range(len(frames)))
        ...
        print(
            "🧠 INSIGHTFACE RECOGNITION FRAMES:",
            [index + 1 for index, _ in recognition_frames],
            flush=True
        )

        # Run only the selected recognition frames concurrently.
        # Recognition thresholds, ArcFace, embeddings, and voting
        ...
        #
        # With the optimized 4-frame recognition set, at least
        # 3 frames must recognize the same employee.
        # This preserves temporal consistency while reducing
        # expensive InsightFace processing.
        # =====================================================
